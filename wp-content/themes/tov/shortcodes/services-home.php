...
function tov_services_home_shortcode($atts) {
    $atts = shortcode_atts([
        'exclude' => '', // Comma-separated list of titles to exclude
    ], $atts);

    // Normalize excluded items
    $excluded = array_map('trim', explode(',', strtolower($atts['exclude'])));
        ...
        <div class="max-w-[1280px] mx-auto px-4 sm:px-6 relative z-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-16 mb-20">
                <?php foreach ($services as $service) : 
                    if (in_array(strtolower($service['title']), $excluded)) {
                        continue;
                    }
                ?>
                <!-- Service: <?php echo esc_html($service['title']); ?> -->
                <div class="group">
                    <a href="<?php echo home_url($service['link']); ?>">
                    <div class="rounded-2xl overflow-hidden mb-6 h-[240px]">
                        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/<?php echo $service['image']; ?>" alt="<?php echo esc_attr($service['title']); ?>" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700">
                    </div>
                    <h3 class="text-[#1C2321] font-jakarta text-2xl font-bold mb-3 hover:text-[#016A7C]"><?php echo esc_html($service['title']); ?></h3>
                    ...
                    <a href="<?php echo home_url($service['link']); ?>" class="bt-l">
                        Learn more
